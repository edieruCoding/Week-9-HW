...
public class CubeGenerator : MonoBehaviour
{
    ...
    public void DrawLines()
    {
        ...
        {
            var reducedVector = center - squareVectors[i];
            reducedVector = RotateX(cubeRotation.x, reducedVector);
            reducedVector = RotateY(cubeRotation.y, reducedVector);
            reducedVector = RotateZ(cubeRotation.z, reducedVector);
            squareVectors[i] = reducedVector + center;
        }
        
        for (int i = 0; i < backsquareVectors.Length; i++)
        {
            var reducedVector = center - backsquareVectors[i];
            reducedVector = RotateX(cubeRotation.x, reducedVector);
            reducedVector = RotateY(cubeRotation.y, reducedVector);
            reducedVector = RotateZ(cubeRotation.z, reducedVector);
            backsquareVectors[i] = reducedVector + center;
        }
