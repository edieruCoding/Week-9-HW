...
public class CylinderGenerator : MonoBehaviour
{
    ...
    private void DrawLines()
    {
        ...
        var botCircle = GetCirclePoints(center.z + cylinderHeight * .5f);

        var topScale = focalLength / ((center.z - cylinderHeight * .5f) + focalLength);
        var botScale =  focalLength/((center.z + cylinderHeight * .5f) + focalLength);
        ...
        for (int i = 0; i < botCircle.Length; i++)
        {
            GL.Color(cylinderMaterial.color);
            var point1 = botCircle[i] * botScale;
            GL.Vertex3(point1.x, point1.y, 0);
            var point2 = botCircle[(i + 1) % botCircle.Length] * botScale;
            GL.Vertex3(point2.x, point2.y, 0);
        }
        ...
        for (int i = 0; i < botCircle.Length; i++)
        {
            ...
            var point1 = topCircle[i] * topScale;
            GL.Vertex3(point1.x, point1.y, 0);
            var point2 = botCircle[i] * botScale;
            GL.Vertex3(point2.x, point2.y, 0);
        }
