...
public class CapsuleGenerator : MonoBehaviour
{
    ...
    private void DrawLines()
    {
        ...
        Vector3[] botFirstRing = null;
        for (int j = 0; j < rings / 2; j++)
        {
            if (j == (rings / 2)-1)
            {
                botFirstRing = DrawRing(j, -capsuleLength / 2);
            }
            
            DrawRing(j, -capsuleLength / 2);
        }

        if (topLastRing != null && botFirstRing != null)
        {
            for (int i = 0; i < topLastRing.Length; i++)
            {
                ...
                var scale2 = focalLength / ((botFirstRing[i].z - center.z) + focalLength); 
                var point2 = botFirstRing[i] * scale2;
